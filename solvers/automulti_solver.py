...
class AutoMultiPatientSolver:
    ...
    def match(self):
        ...
        for i in range(n):
            solver.Add(
                solver.Sum(x[i, j] for j in range(m)) <= self.nurses[i].max_patients
            )
        num_patients = {}
        for i in range(n):
            num_patients[i] = solver.IntVar(
                0, self.nurses[i].max_patients, f"num_patients[{i}]"
            )
        ...
        for i in range(n):
            total_stress = (
                self.nurses[i].current_stress + stress_per_assignment * num_patients[i]
            )
        ...
        for i in range(n):
            ...
            objective.SetCoefficient(
                num_patients[i], -stress_penalty_weight * self.nurses[i].current_stress
            )
        ...
        if status in [pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE]:
            ...
            print("Feasible Assignment found: ")
            for i in range(n):
                for j in range(m):
                    if x[i, j].solution_value() > 0.5:
                        nurse = self.nurses[i]
